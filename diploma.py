...
TOKEN = '1c9872eff7212c447f2823d25c23f062a52da9bc8cb540fcd70d582b15e9ee6e24f5705e267b31dfd0802'
                        ...
                        if 'interests' in s_params_input:
                            ...
                            if person['interests']:
                                ...
                                if result is None:
                                    ...
                                    if counter < 10 and person_id not in previous_results:
    ...
    def add_top3_photos_json(self):
        ...
        for person in final_list:
            ...
            if 'error' in resp_json and 'error_code' in resp_json['error'] and \
                ...
                n_resp_json = response1.json()
                while 'error' in n_resp_json and 'error_code' in n_resp_json['error'] and \
                        n_resp_json['error']['error_code'] == 6:
                    time.sleep(1)
                top_dict['1']['link'] = n_resp_json['response'][0]['photo_max_orig']
                continue
            while 'error' in resp_json and 'error_code' in resp_json['error'] and \
